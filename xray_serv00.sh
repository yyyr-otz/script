...
export UUID=${UUID:-'31d27103-51e8-4c69-bef5-f416aa5ae62d'}  
export NEZHA_SERVER=${NEZHA_SERVER:-'{oec-cf.yyyrspeed.nyc.mn:443'}  # v1哪吒形式：nezha.abc.com:8008,v0哪吒形式：nezha.abc.com
export NEZHA_PORT=${NEZHA_PORT:-''}      # v1哪吒不需要此变量
export NEZHA_KEY=${NEZHA_KEY:-'N2nF4h3f7WkOoV1JI4YdJLnJ59DqSqW9'}        # v1的NZ_CLIENT_SECRET或v0的agent密钥
export ARGO_DOMAIN=${ARGO_DOMAIN:-'pl-serv00.yyyrspeed.nyc.mn'}   
export ARGO_AUTH=${ARGO_AUTH:-'eyJhIjoiZjJjNzZmNjcxYWZkODcxNzc1NjUyMjFjZjYwOWY4MWEiLCJ0IjoiMDFkZDkxYjEtYzM2MS00OTFjLWI3YmItZGU4Yjc4ZTJjMmE4IiwicyI6Ik56aGlPRFJtWTJNdFpETTRaUzAwWW1JM0xXRTJNREF0WVRka1pUTm1ZV1k0TnpOayJ9'}
export CFIP=${CFIP:-'192.168.0.99'} 
export CFPORT=${CFPORT:-'18443'}
...
EOF
    ...
    yellow "\n当前使用的是token,请在cloudflare后台设置隧道端口为${purple}${XRAY_PORT}${re}"
  fi
}
  ...
  "inbounds": [
    {
      ...
      "settings": {
        ...
        "decryption": "none",
        "fallbacks": [
           {"alpn": "","dest": "/dev/shm/ws.sock","name": "","path": "/ws","xver": 0}, 
		   {"alpn": "","dest": "/dev/shm/xhttp.sock","name": "","path": "","xver": 0},
        ]
      },
      "streamSettings": { "network": "tcp" }
    },
    {
      "listen": "/dev/shm/ws.sock,0666", "protocol": "vless",
      "settings": { "clients": [{ "id": "${UUID}", "level": 0 }], "decryption": "none" },
      "streamSettings": { "network": "ws", "security": "none", "wsSettings": { "path": "/ws" } },
      "sniffing": { "enabled": true, "destOverride": ["http", "tls", "quic"], "metadataOnly": false }
    },
	{
      "listen": "/dev/shm/xh.sock,0666", "protocol": "vless",
      "settings": { "clients": [{ "id": "${UUID}", "level": 0 }], "decryption": "none" },
...
if [ "$ARCH" == "arm" ] || [ "$ARCH" == "arm64" ] || [ "$ARCH" == "aarch64" ]; then
	XRAY_URL="https://github.com/yyyr-otz/script/releases/download/freebsd-amd64/xray"
    BASE_URL="https://github.com/eooce/test/releases/download/freebsd-arm64"
elif [ "$ARCH" == "amd64" ] || [ "$ARCH" == "x86_64" ] || [ "$ARCH" == "x86" ]; then
	XRAY_URL="https://github.com/yyyr-otz/script/releases/download/freebsd-arm64/xray"
    BASE_URL="https://github.com/eooce/test/releases/download/freebsd"
else
    echo "Unsupported architecture: $ARCH"
    exit 1
fi
FILE_INFO=("$XRAY_URL web" "$BASE_URL/server bot")
...
uuid: $(echo -n "$USERNAME+$HOSTNAME" | md5sum | head -c 32 | sed -E 's/(.{8})(.{4})(.{4})(.{4})(.{12})/\1-\2-\3-\4-\5/')
EOF
fi
declare -A FILE_MAP
...
if [ -e "$(basename ${FILE_MAP[bot]})" ]; then
    if [[ $ARGO_AUTH =~ ^[A-Z0-9a-z=]{120,250}$ ]]; then
      ...
      args="tunnel --edge-ip-version auto --no-autoupdate --protocol http2 --logfile boot.log --loglevel info --url http://localhost:$XRAY_PORT"
    fi
...
vless://${UUID}@${CFIP}:${CFPORT}?encryption=none&security=tls&sni=${argodomain}&alpn=h3%2Ch2%2Chttp%2F1.1&fp=chrome&type=xhttp&host=${argodomain}&path=%2Fxh&mode=auto#argo%2Bxh
EOF
