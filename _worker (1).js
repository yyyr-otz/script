...
let addresses = [
	// 'us2.yyyrspeed.us.kg#自建',
	'visa.cn#date',
	'cf.090227.xyz#三网',
	'ct.090227.xyz#电信',
	'cmcc.090227.xyz#移动',
	'cfip.xxxxxxxx.tk#OTC官方',
	'bestcf.onecf.eu.org#Mingyu官方',
	'cf.zhetengsha.eu.org#小一官方',
	'bestproxy.onecf.eu.org#Mingyu反代',
	'proxy.xxxxxxxx.tk#OTC反代',
	// 'edgetunnel.anycast.eu.org#反代ed',
	// 'ts.hpc.tw#反代hpc',
	// 'cdn.xn--b6gac.eu.org#反代b6',
	// 'cdn-all.xn--b6gac.eu.org#反代b6',
];

// 设置优选地址api接口
let addressesapi = [
	'https://txt.932624033.workers.dev/cdnyx.txt?token=yyyr',
...
let subconfig = "https://raw.githubusercontent.com/yyyr-otz/script/master/ACL4SSR/clash/cfcdn_config.ini"; //订阅转换配置文件
	...
	async fetch (request, env) {
		...
		*/
		...
		await sendMessage("#VLESS订阅", request.headers.get('CF-Connecting-IP'), `UA: ${userAgentHeader}</tg-spoiler>\n域名: ${url.hostname}\n<tg-spoiler>入口: ${url.pathname + url.search}</tg-spoiler>`);
			...
			urltls = url.searchParams.get('tls') || urltls;
		...
		if (!userAgent.includes('subconverter') && MamaJustKilledAMan.some(PutAGunAgainstHisHeadPulledMyTriggerNowHesDead => userAgent.includes(PutAGunAgainstHisHeadPulledMyTriggerNowHesDead)) && MamaJustKilledAMan.length > 0) {
				...
				notlsresponseBody = uniqueAddressesnotls.map(address => {
					...
					if (!match) {
						if (address.includes(':') && address.includes('#')) {
							...
							address = parts[0];
							port = parts[1];
						} else if (address.includes('date')) {
						const parts = address.split('#');
						address = parts[0];
						// 获取当前上海时间并格式化为 "月-日 时:分"
							// const dateoptions = { timeZone: 'Asia/Shanghai', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit' };
							// const shanghaiTime = new Date().toLocaleString('zh-CN', dateoptions).replace(',', '').replace(/\//g, '-');
							// let shanghaiTime = new Date().toLocaleString('zh-CN', dateoptions).replace(',', '');
							// shanghaiTime = shanghaiTime.replace(/\//g, '-').replace(' ', ' ');
							// addressid = shanghaiTime;
							const dateoptions = { timeZone: 'Asia/Shanghai', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', hour12: false };
							const shanghaiTime = new Intl.DateTimeFormat('zh-CN', dateoptions).format(new Date());
							// 将日期和时间格式化为 "xx月xx日 xx时xx分"
							const [monthDay, time] = shanghaiTime.split(' ');
							const [month, day] = monthDay.split('/');
							const [hour, minute] = time.split(':');
							addressid = `订阅时间 ${month}月${day}日 ${hour}时${minute}分`;
					        } else if (address.includes('#')) {
							const parts = address.split('#');
							address = parts[0];
							addressid = parts[1];
					...
					const vlessLink = `vless://${uuid}@${address}:${port}?encryption=none&security=&type=${type}&host=${host}&path=${encodeURIComponent(path)}#${encodeURIComponent(addressid + EndPS)}`;
			                // const vlessLink = `vless://${uuid}@${address}:8080?encryption=none&security=&type=${type}&host=${host}&path=${encodeURIComponent(path)}#${encodeURIComponent(addressid + EndPS)}`;
					return vlessLink;
			...
			if (notlsresponseBody && noTLS == 'true') {
				combinedContent = notlsresponseBody;
