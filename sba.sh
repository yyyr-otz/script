...
# 创建快捷方式
create_shortcut() {
  cat > $WORK_DIR/sba.sh << EOF
#!/usr/bin/env bash

bash <(wget --no-check-certificate -qO- https://raw.githubusercontent.com/yyyr-otz/ymal/master/sba.sh) \$1
EOF
  chmod +x $WORK_DIR/sba.sh
  ln -sf $WORK_DIR/sba.sh /usr/bin/sba
