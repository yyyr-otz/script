...
E[5]="The script supports Debian, Ubuntu, CentOS, Alpine or Arch systems only. Feedback: [https://github.com/yyyr-otz/script/issues]"
C[5]="本脚本只支持 Debian、Ubuntu、CentOS、Alpine 或 Arch 系统,问题反馈:[https://github.com/yyyr-otz/script/issues]"
E[6]="Curren operating system is \$SYS.\\\n The system lower than \$SYSTEM \${MAJOR[int]} is not supported. Feedback: [https://github.com/yyyr-otz/script/issues]"
C[6]="当前操作是 \$SYS\\\n 不支持 \$SYSTEM \${MAJOR[int]} 以下系统,问题反馈:[https://github.com/yyyr-otz/script/issues]"
...
E[24]="Choose:"
C[24]="请选择:"
E[25]="Curren architecture \$(uname -m) is not supported. Feedback: [https://github.com/yyyr-otz/script/issues]"
C[25]="当前架构 \$(uname -m) 暂不支持,问题反馈:[https://github.com/yyyr-otz/script/issues]"
E[26]="Not install"
C[26]="未安装"
E[27]="close"
C[27]="关闭"
...
C[44]="不需要升级"
E[45]="Argo authentication message does not match the rules, neither Token nor Json, script exits. Feedback:[https://github.com/yyyr-otz/script/issues]"
C[45]="Argo 认证信息不符合规则，既不是 Token，也是不是 Json，脚本退出，问题反馈:[https://github.com/yyyr-otz/script/issues]"
E[46]="Connect"
C[46]="连接"
E[47]="The script must be run as root, you can enter sudo -i and then download and run again. Feedback:[https://github.com/yyyr-otz/script/issues]"
C[47]="必须以root方式运行脚本，可以输入 sudo -i 后重新下载运行，问题反馈:[https://github.com/yyyr-otz/script/issues]"
E[48]="Downloading the latest version \$APP failed, script exits. Feedback:[https://github.com/yyyr-otz/script/issues]"
C[48]="下载最新版本 \$APP 失败，脚本退出，问题反馈:[https://github.com/yyyr-otz/script/issues]"
...
check_cdn() {
  [ -n "$GH_PROXY" ] && wget --server-response --quiet --output-document=/dev/null --no-check-certificate --tries=2 --timeout=3 https://raw.githubusercontent.com/yyyr-otz/script/main/README.md >/dev/null 2>&1 && unset GH_PROXY
}
...
statistics_of_run-times() {
  local COUNT=$(wget --no-check-certificate -qO- --tries=2 --timeout=2 "https://hit.forvps.gq/https://raw.githubusercontent.com/yyyr-otz/script/main/argox.sh" 2>&1 | grep -m1 -oE "[0-9]+[ ]+/[ ]+[0-9]+") &&
...
bash <(wget --no-check-certificate -qO- ${GH_PROXY}https://raw.githubusercontent.com/yyyr-otz/script/main/argox.sh) \$1
EOF
