...
if [ ! -f sbmg_update ]; then
    green "首次安装Sing-box-mg脚本必要的依赖……"
    update() {
...
inscertificate() {
    ymzs() {
        ...
        blue "Vmess-ws将开启TLS，Naive-proxy、Hysteria-1、Hysteria-2、Tuic-v5将使用 $(cat /root/ygkkkca/ca.log 2>/dev/null) 证书，并开启SNI证书验证"
        tlsyn=true
        ym_vm_ws=$(cat /root/ygkkkca/ca.log 2>/dev/null)
        certificatec_vmess_ws='/root/ygkkkca/cert.crt'
        certificatep_vmess_ws='/root/ygkkkca/private.key'
        certificatec_naive='/root/ygkkkca/cert.crt'
        certificatep_naive='/root/ygkkkca/private.key'
        certificatec_hy1='/root/ygkkkca/cert.crt'
        certificatep_hy1='/root/ygkkkca/private.key'
    ...
    zqzs() {
        ym_vl_re=www.yahoo.com
        blue "Vless-reality的SNI域名默认为 www.yahoo.com"
        blue "Vmess-ws将关闭TLS，Naive-proxy、Hysteria-1、Hysteria-2、Tuic-v5将使用bing自签证书，并关闭SNI证书验证"
        tlsyn=false
        ym_vm_ws=www.bing.com
        certificatec_vmess_ws='/etc/s-box/cert.pem'
        certificatep_vmess_ws='/etc/s-box/private.key'
        certificatec_naive='/etc/s-box/cert.pem'
        certificatep_naive='/etc/s-box/private.key'
        certificatec_hy1='/etc/s-box/cert.pem'
        certificatep_hy1='/etc/s-box/private.key'
...
naiveport() {
    readp "\n设置Naiveproxy主端口[1-65535] (回车跳过为2000-65535之间的随机端口)：" port
    chooseport
    port_naive=$port
}
hy1port() {
    readp "\n设置Hysetria1主端口[1-65535] (回车跳过为2000-65535之间的随机端口)：" port
    chooseport
    port_hy1=$port
}
hy2port() {
...
insport() {
    ...
    if [ -z "$port" ] || [ "$port" = "1" ]; then
        ...
        vlport && vmport && naiveport && hy1port && hy2port && tu5port
    fi
    echo
    blue "各协议端口确认如下"
    blue "Vless-reality端口：$port_vl_re"
    blue "Vmess-ws端口：$port_vm_ws"
    blue "Naive-proxy端口：$port_naive"
    blue "Hysteria-1端口：$port_hy1"
...
result_vl_vm_na_hy1_hy2_tu() {
    ...
    naive_port=$(jq -r '.inbounds[2].listen_port' /etc/s-box/sb.json)
    ym=$(cat /root/ygkkkca/ca.log 2>/dev/null)
    naive_sniname=$(jq -r '.inbounds[2].tls.key_path' /etc/s-box/sb.json)
    if [[ "$naive_sniname" = '/etc/s-box/private.key' ]]; then
        naive_name=www.bing.com
        sb_tnaive_ip=$server_ip
        cl_naive_ip=$server_ipcl
        ins=1
        naive_ins=true
    else
        naive_name=$ym
    ...
    hy1_port=$(jq -r '.inbounds[3].listen_port' /etc/s-box/sb.json)
    hy1_ports=$(iptables -t nat -nL --line 2>/dev/null | grep -w "$hy2_port" | awk '{print $8}' | sed 's/dpts://; s/dpt://' | tr '\n' ',' | sed 's/,$//')
    if [[ -n $hy1_ports ]]; then
        hy1ports=$(echo $hy1_ports | sed 's/:/-/g')
        hy1ps=$hy1_port,$hy1ports
    else
        hy1ps=$hy1_port
    fi
    ym=$(cat /root/ygkkkca/ca.log 2>/dev/null)
    hy1_sniname=$(jq -r '.inbounds[3].tls.key_path' /etc/s-box/sb.json)
    if [[ "$hy1_sniname" = '/etc/s-box/private.key' ]]; then
        hy1_name=www.bing.com
        sb_hy1_ip=$server_ip
        cl_hy1_ip=$server_ipcl
        ins_hy1=1
        hy1_ins=true
    else
        hy1_name=$ym
    ...
    hy2_port=$(jq -r '.inbounds[4].listen_port' /etc/s-box/sb.json)
    hy2_ports=$(iptables -t nat -nL --line 2>/dev/null | grep -w "$hy2_port" | awk '{print $8}' | sed 's/dpts://; s/dpt://' | tr '\n' ',' | sed 's/,$//')
    if [[ -n $hy2_ports ]]; then
        hy2ports=$(echo $hy2_ports | sed 's/:/-/g')
        hy2ps=$hy2_port,$hy2ports
    else
        hy2ps=$hy2_port
    fi
    ym=$(cat /root/ygkkkca/ca.log 2>/dev/null)
    hy2_sniname=$(jq -r '.inbounds[4].tls.key_path' /etc/s-box/sb.json)
    ...
    tu5_port=$(jq -r '.inbounds[5].listen_port' /etc/s-box/sb.json)
    ym=$(cat /root/ygkkkca/ca.log 2>/dev/null)
    tu5_sniname=$(jq -r '.inbounds[5].tls.key_path' /etc/s-box/sb.json)
...
resvless() {
    echo
    white "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
    vl_link="vless://$uuid@$server_ip:$vl_port?encryption=none&flow=xtls-rprx-vision&security=reality&sni=$vl_name&fp=chrome&pbk=$public_key&sid=$short_id&type=tcp&headerType=none#mg-vl-reality"
...
resvmess() {
    if [[ "$tls" = "false" ]]; then
        if [[ -n $(ps -ef | grep cloudflared) && -s '/etc/s-box/argo.log' ]]; then
            ...
            echo
            echo "分享链接【v2rayn、v2rayng、nekobox、小火箭shadowrocket】"
            echo -e "${yellow}vmess://$(echo '{"add":"www.visa.com","aid":"0","host":"'$argo'","id":"'$uuid'","net":"ws","path":"'$ws_path'","port":"443","ps":"mg-vm-argo","tls":"tls","sni":"'$argo'","type":"none","v":"2"}' | base64 -w 0)${plain}"
            echo
            echo "二维码【v2rayn、v2rayng、nekobox、小火箭shadowrocket】"
            echo 'vmess://'$(echo '{"add":"www.visa.com","aid":"0","host":"'$argo'","id":"'$uuid'","net":"ws","path":"'$ws_path'","port":"443","ps":"mg-vm-argo","tls":"tls","sni":"'$argo'","type":"none","v":"2"}' | base64 -w 0) >/etc/s-box/vm_ws_argo.txt
        ...
        echo
        echo "分享链接【v2rayn、v2rayng、nekobox、小火箭shadowrocket】"
        echo -e "${yellow}vmess://$(echo '{"add":"'$server_ip'","aid":"0","host":"'$vm_name'","id":"'$uuid'","net":"ws","path":"'$ws_path'","port":"'$vm_port'","ps":"mg-vm-ws","tls":"","type":"none","v":"2"}' | base64 -w 0)${plain}"
        echo
        echo "二维码【v2rayn、v2rayng、nekobox、小火箭shadowrocket】"
        echo 'vmess://'$(echo '{"add":"'$server_ip'","aid":"0","host":"'$vm_name'","id":"'$uuid'","net":"ws","path":"'$ws_path'","port":"'$vm_port'","ps":"mg-vm-ws","tls":"","type":"none","v":"2"}' | base64 -w 0) >/etc/s-box/vm_ws.txt
        ...
        echo
        echo "分享链接【v2rayn、v2rayng、nekobox、小火箭shadowrocket】"
        echo -e "${yellow}vmess://$(echo '{"add":"'$vm_name'","aid":"0","host":"'$vm_name'","id":"'$uuid'","net":"ws","path":"'$ws_path'","port":"'$vm_port'","ps":"mg-vm-ws-tls","tls":"tls","sni":"'$vm_name'","type":"none","v":"2"}' | base64 -w 0)${plain}"
        echo
        echo "二维码【v2rayn、v2rayng、nekobox、小火箭shadowrocket】"
        echo 'vmess://'$(echo '{"add":"'$vm_name'","aid":"0","host":"'$vm_name'","id":"'$uuid'","net":"ws","path":"'$ws_path'","port":"'$vm_port'","ps":"mg-vm-ws-tls","tls":"tls","sni":"'$vm_name'","type":"none","v":"2"}' | base64 -w 0) >/etc/s-box/vm_ws_tls.txt
...
resna() {
    echo
    white "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
    naive_link="naive+quic://$uuid:$uuid@vps.naive:$naive_port#mg-naive"
    echo "$naive_link" >/etc/s-box/naive.txt
    red "🚀【 Naive-proxy 】节点信息如下：" && sleep 2
    echo
    echo "分享链接【nekobox、小火箭shadowrocket】"
    echo -e "${yellow}$naive_link${plain}"
    echo
    echo "二维码【nekobox、小火箭shadowrocket】"
    qrencode -o - -t ANSIUTF8 "$(cat /etc/s-box/naive.txt)"
    white "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
    echo
}
reshy1() {
    echo
    white "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
    hy1_link="hysteria://$sb_hy1_ip:$hy1_port?upmbps=9&downmbps=45&auth=$uuid&mport=$hy1ps&insecure=$ins_hy1&peer=$hy1_name&alpn=h3#mg-hy1"
    echo "$hy1_link" >/etc/s-box/hy1.txt
    red "🚀【 Hysteria-1 】节点信息如下：" && sleep 2
    echo
    echo "分享链接【nekobox、小火箭shadowrocket】"
    echo -e "${yellow}$hy1_link${plain}"
    echo
    echo "二维码【nekobox、小火箭shadowrocket】"
    qrencode -o - -t ANSIUTF8 "$(cat /etc/s-box/hy1.txt)"
    white "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
    echo
}
reshy2() {
    echo
    white "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
    hy2_link="hysteria2://$uuid@$sb_hy2_ip:$hy2_port?insecure=$ins_hy2&mport=$hy2ps&sni=$hy2_name#mg-hy2"
...
restu5() {
    echo
    white "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
    tuic5_link="tuic://$uuid:$uuid@$sb_tu5_ip:$tu5_port?congestion_control=bbr&udp_relay_mode=native&alpn=h3&sni=$tu5_name&allow_insecure=$ins#mg-tu5"
...
instsllsingbox() {
    ...
    clear
    red "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
    lnmg && blue "Sing-box安装成功，脚本快捷方式为 mg" && cronsb
    sbshare
    red "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
    blue "Naive-Proxy、Hysetria-1、Hysteria2、Tuic5的v2rayn配置文件、Clash-Meta、SFA/SFI/SFW客户端配置文件，请选择9进行查看"
    red "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
    echo
}
changeym() {
    ...
    naive_sniname=$(jq -r '.inbounds[2].tls.key_path' /etc/s-box/sb.json)
    [[ "$_sniname" = '/etc/s-box/private.key' ]] && naive_na="正在使用自签bing证书。$ymzs" || naive_na="正在使用的域名证书：$(cat /root/ygkkkca/ca.log 2>/dev/null)。$yellow切换为自签bing证书$plain"
    hy1_sniname=$(jq -r '.inbounds[3].tls.key_path' /etc/s-box/sb.json)
    [[ "$hy1_sniname" = '/etc/s-box/private.key' ]] && hy1_na="正在使用自签bing证书。$ymzs" || hy1_na="正在使用的域名证书：$(cat /root/ygkkkca/ca.log 2>/dev/null)。$yellow切换为自签bing证书$plain"
    hy2_sniname=$(jq -r '.inbounds[4].tls.key_path' /etc/s-box/sb.json)
    ...
    if [[ -f /root/ygkkkca/ca.log ]]; then
        ...
        green "3：Naiveproxy协议，$naive_na"
        green "4：Hysteria1协议，$hy1_na"
        green "5：Hysteria2协议，$hy2_na"
        green "6：Tuic5协议，$tu5_na"
    else
        red "仅支持选项1 (vless-reality)。因未申请域名证书，vmess-ws、Naiveproxy、Hysteria-1、Hysteria-2、Tuic-v5的证书切换选项暂不予显示"
    fi
    green "0：返回上层"
    readp "请选择：" menu
    if [ "$menu" = "1" ]; then
        ...
        result_vl_vm_na_hy1_hy2_tu && resvless && sb_client
    elif [ "$menu" = "2" ]; then
        if [ -f /root/ygkkkca/ca.log ]; then
            ...
            systemctl restart sing-box
            result_vl_vm_na_hy1_hy2_tu && resvmess && sb_client
        else
            red "当前未申请域名证书，不可切换。主菜单选择12，执行Acme证书申请" && sleep 2 && mg
        fi
    elif [ "$menu" = "3" ]; then
        if [ -f /root/ygkkkca/ca.log ]; then
            ...
            systemctl restart sing-box
            result_vl_vm_na_hy1_hy2_tu && resna && sb_client
        else
            red "当前未申请域名证书，不可切换。主菜单选择12，执行Acme证书申请" && sleep 2 && mg
        fi
    elif [ "$menu" = "4" ]; then
        if [ -f /root/ygkkkca/ca.log ]; then
            ...
            systemctl restart sing-box
            result_vl_vm_na_hy1_hy2_tu && reshy1 && sb_client
        else
            red "当前未申请域名证书，不可切换。主菜单选择12，执行Acme证书申请" && sleep 2 && mg
        fi
    elif [ "$menu" = "5" ]; then
        if [ -f /root/ygkkkca/ca.log ]; then
            c=$(jq -r '.inbounds[4].tls.certificate_path' /etc/s-box/sb.json)
            d=$(jq -r '.inbounds[4].tls.key_path' /etc/s-box/sb.json)
            if [ "$d" = '/etc/s-box/private.key' ]; then
                c_c='/root/ygkkkca/cert.crt'
                d_d='/root/ygkkkca/private.key'
            else
            ...
            sed -i "82s#$d#$d_d#" /etc/s-box/sb.json
            systemctl restart sing-box
            result_vl_vm_na_hy1_hy2_tu && reshy2 && sb_client
        else
            red "当前未申请域名证书，不可切换。主菜单选择12，执行Acme证书申请" && sleep 2 && mg
        fi
    elif [ "$menu" = "6" ]; then
        if [ -f /root/ygkkkca/ca.log ]; then
            c=$(jq -r '.inbounds[5].tls.certificate_path' /etc/s-box/sb.json)
            d=$(jq -r '.inbounds[5].tls.key_path' /etc/s-box/sb.json)
            if [ "$d" = '/etc/s-box/private.key' ]; then
                c_c='/root/ygkkkca/cert.crt'
                d_d='/root/ygkkkca/private.key'
            else
            ...
            sed -i "105s#$d#$d_d#" /etc/s-box/sb.json
            systemctl restart sing-box
            result_vl_vm_na_hy1_hy2_tu && restu5 && sb_client
        else
            red "当前未申请域名证书，不可切换。主菜单选择12，执行Acme证书申请" && sleep 2 && mg
        fi
    else
...
allports() {
    ...
    hy2_port=$(jq -r '.inbounds[4].listen_port' /etc/s-box/sb.json)
    tu5_port=$(jq -r '.inbounds[5].listen_port' /etc/s-box/sb.json)
    naive_ports=$(iptables -t nat -nL --line 2>/dev/null | grep -w "$naive_port" | awk '{print $8}' | sed 's/dpts://; s/dpt://' | tr '\n' ',' | sed 's/,$//')
    hy1_ports=$(iptables -t nat -nL --line 2>/dev/null | grep -w "$hy1_port" | awk '{print $8}' | sed 's/dpts://; s/dpt://' | tr '\n' ',' | sed 's/,$//')
    hy2_ports=$(iptables -t nat -nL --line 2>/dev/null | grep -w "$hy2_port" | awk '{print $8}' | sed 's/dpts://; s/dpt://' | tr '\n' ',' | sed 's/,$//')
    tu5_ports=$(iptables -t nat -nL --line 2>/dev/null | grep -w "$tu5_port" | awk '{print $8}' | sed 's/dpts://; s/dpt://' | tr '\n' ',' | sed 's/,$//')
    [[ -n $naive_ports ]] && naivezfport="$naive_ports" || naivezfport="未添加"
    [[ -n $hy1_ports ]] && hy1zfport="$hy1_ports" || hy1zfport="未添加"
...
changeport() {
    ...
    naivedeports() {
        allports
        naiv_ports=$(echo "$naive_ports" | sed 's/,/,/g')
        IFS=',' read -ra ports <<<"$naive_ports"
        for port in "${ports[@]}"; do
            iptables -t nat -D PREROUTING -p udp --dport $port -j DNAT --to-destination :$naive_port
            ip6tables -t nat -D PREROUTING -p udp --dport $port -j DNAT --to-destination :$naive_port
        done
        netfilter-persistent save >/dev/null 2>&1
        service iptables save >/dev/null 2>&1
    }
    hy1deports() {
        allports
        hy1_ports=$(echo "$hy1_ports" | sed 's/,/,/g')
        IFS=',' read -ra ports <<<"$hy1_ports"
        for port in "${ports[@]}"; do
            iptables -t nat -D PREROUTING -p udp --dport $port -j DNAT --to-destination :$hy1_port
            ip6tables -t nat -D PREROUTING -p udp --dport $port -j DNAT --to-destination :$hy1_port
        done
        netfilter-persistent save >/dev/null 2>&1
        service iptables save >/dev/null 2>&1
    }
    ...
    green "Naiveproxy、Hysetiea1、Hysteria2、Tuic5支持更改主端口，也支持增删多个转发端口"
    green "Hysetiea1、Hysteria2支持端口跳跃，且与Tuic5都支持多端口复用"
    green "1：vless-reality协议 ${yellow}端口:$vl_port${plain}"
    green "2：vmess-ws协议 ${yellow}端口:$vm_port${plain}"
    green "3：Naiveproxy协议 ${yellow}端口:$naive_port  转发多端口: $naivezfport${plain}"
    green "4：Hysteria1协议 ${yellow}端口:$hy1_port  转发多端口: $hy1zfport${plain}"
    green "5：Hysteria2协议 ${yellow}端口:$hy2_port  转发多端口: $hy2zfport${plain}"
    green "6：Tuic5协议 ${yellow}端口:$tu5_port  转发多端口: $tu5zfport${plain}"
    green "0：返回上层"
    readp "请选择要变更端口的协议【0-6】：" menu
    if [ "$menu" = "1" ]; then
        vlport
        ...
        green "1：更换Naiveproxy主端口 (原多端口自动重置删除)"
        green "2：添加Naiveproxy多端口"
        green "3：重置删除Naiveproxy多端口"
        green "0：返回上层"
        readp "请选择【0-3】：" menu
        if [ "$menu" = "1" ]; then
            if [ -n $naive_ports ]; then
                naivedeports
                naiveport
                sed -i "91s/$naive_port/$port_naive/" /etc/s-box/sb.json
                systemctl restart sing-box
                result_vl_vm_na_hy1_hy2_tu && resnaive && sb_client
            else
                naiveport
                sed -i "91s/$naive_port/$port_naive/" /etc/s-box/sb.json
                systemctl restart sing-box
            ...
            green "1：添加Naiveproxy范围端口"
            green "2：添加Naiveproxy单端口"
            green "0：返回上层"
            readp "请选择【0-2】：" menu
            if [ "$menu" = "1" ]; then
                port=$(jq -r '.inbounds[2].listen_port' /etc/s-box/sb.json)
                fports && result_vl_vm_na_hy1_hy2_tu && sb_client && changeport
            elif [ "$menu" = "2" ]; then
                port=$(jq -r '.inbounds[2].listen_port' /etc/s-box/sb.json)
                fport && result_vl_vm_na_hy1_hy2_tu && sb_client && changeport
            else
                changeport
            fi
        elif [ "$menu" = "3" ]; then
            if [ -n $naive_ports ]; then
                naivedeports && result_vl_vm_na_hy1_hy2_tu && sb_client && changeport
            else
                yellow "Naiveproxy未设置多端口" && changeport
            fi
        else
            changeport
        fi
    elif [ "$menu" = "4" ]; then
        green "1：更换Hysteria1主端口 (原多端口自动重置删除)"
        green "2：添加Hysteria1多端口"
        green "3：重置删除Hysteria1多端口"
        green "0：返回上层"
        readp "请选择【0-3】：" menu
        if [ "$menu" = "1" ]; then
            if [ -n $hy1_ports ]; then
                hy1deports
                hy1port
                sed -i "67s/$hy1_port/$port_hy1/" /etc/s-box/sb.json
                systemctl restart sing-box
                result_vl_vm_na_hy1_hy1_tu && reshy1 && sb_client
            else
                hy1port
                sed -i "67s/$hy1_port/$port_hy1/" /etc/s-box/sb.json
                systemctl restart sing-box
            ...
            green "1：添加Hysteria1范围端口"
            green "2：添加Hysteria1单端口"
            green "0：返回上层"
            readp "请选择【0-2】：" menu
            if [ "$menu" = "1" ]; then
                port=$(jq -r '.inbounds[2].listen_port' /etc/s-box/sb.json)
            ...
            if [ -n $hy1_ports ]; then
                hy1deports && result_vl_vm_na_hy1_hy2_tu && sb_client && changeport
            else
                yellow "Hysteria1未设置多端口" && changeport
            fi
        else
            changeport
        fi     
        ...
        if [ "$menu" = "1" ]; then
            ...
            if [ -n $hy2_ports ]; then
                hy2deports && result_vl_vm_na_hy1_hy2_tu && sb_client && changeport
            else
        ...
        if [ "$menu" = "1" ]; then
            ...
            if [ -n $tu5_ports ]; then
                tu5deports && result_vl_vm_na_hy1_hy2_tu && sb_client && changeport
            else
...
tgnotice() {
    if [[ -f /etc/s-box/sbtg.sh ]]; then
        ...
        red "未设置TG通知功能，请前往主菜单选项3进行设置" && sleep 2 && mg
    fi
}
changeserv() {
    sbactive
...
lnmg() {
    curl -sL -o /usr/bin/smg https://raw.githubusercontent.com/yyyr-otz/ymal/master/mogai.sh
    chmod +x /usr/bin/mg
}
upsbmg() {
    if [[ ! -f '/usr/bin/mg' ]]; then
        red "未正常安装Sing-box-mg" && exit
    fi
    lnmg
    curl -sL https://raw.githubusercontent.com/yyyr-otz/ymal/master/mogai.sh | awk -F "更新内容" '{print $1}' | head -n 1 >/etc/s-box/v
    green "Sing-box-mg安装脚本升级成功" && sleep 5 && sb
}
lapre() {
...
upsbcroe() {
    ...
    if [[ -f '/etc/s-box/sing-box.tar.gz' ]]; then
        ...
        if [[ -f '/etc/s-box/sing-box' ]]; then
            ...
            chmod +x /etc/s-box/sing-box
            systemctl restart sing-box
            blue "成功升级/切换 Sing-box 内核版本：$(/etc/s-box/sing-box version | awk '/version/{print $NF}')" && sleep 3 && mg
        else
...
unins() {
    ...
    rm -rf /etc/s-box sbmg_update /usr/bin/sb /root/geosite.db /root/geoip.db
    uncronsb
...
sbshare() {
    result_vl_vm_na_hy1_hy2_tu && resvless && resvmess && reshy2 && restu5 && sb_client
}
clash_sb_share() {
    echo
    yellow "1：查看最新各协议分享链接、二维码"
    yellow "2：查看最新Clash-Meta、Sing-box客户端SFA/SFI/SFW统一配置文件"
    yellow "3：查看最新Naive、Hysteria1、Hysteria2、Tuic5的V2rayN客户端配置文件"
...
showprotocol() {
    ...
    echo -e "🚀【  Naiveproxy   】${yellow}端口:$naive_port  证书形式:$naive_zs  转发多端口: $naivezfport${plain}"
    echo -e "🚀【  Hysteria-1   】${yellow}端口:$hy1_port  证书形式:$hy1_zs  转发多端口: $hy1zfport${plain}"
...
white "魔改整活版——魔改整活版——魔改整活版——魔改整活版——魔改整活版——魔改整活版——魔改整活版——魔改整活版"
white "魔改整活版——魔改整活版——魔改整活版——魔改整活版——魔改整活版——魔改整活版——魔改整活版——魔改整活版"
white "魔改整活版——魔改整活版——魔改整活版——魔改整活版——魔改整活版——魔改整活版——魔改整活版——魔改整活版"
white "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
white "Vless-reality-vision、Vmess-ws(tls)+Argo、Naiveproxy、Hysteria-1、Hysteria-2、Tuic-v5 一键六协议共存"
white "Sing-box-mg脚本快捷方式：mg"
red "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
green " 1. 安装 Sing-box"
green " 2. 卸载 Sing-box"
white "----------------------------------------------------------------------------------"
