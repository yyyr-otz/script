...
checktls() {
    if [[ -f /root/acme/cert.crt && -f /root/acme/private.key ]] && [[ -s /root/acme/cert.crt && -s /root/acme/private.key ]]; then
        cronac
        green "域名证书申请成功或已存在！域名证书（cert.crt）和密钥（private.key）已保存到 /root/acme文件夹内"
        yellow "公钥文件crt路径如下，可直接复制"
        green "/root/acme/cert.crt"
        yellow "密钥文件key路径如下，可直接复制"
        green "/root/acme/private.key"
        echo $ym >/root/acme/ca.log
...
installCA() {
    bash ~/.acme.sh/acme.sh --install-cert -d ${ym} --key-file /root/acme/private.key --fullchain-file /root/acme/cert.crt --ecc
}
checkip() {
    v4v6
    if [[ -z $v4 ]]; then
...
acmeshow() {
    if [[ -n $(~/.acme.sh/acme.sh -v 2>/dev/null) ]]; then
        ...
        if [[ -n $caacme1 && ! $caacme1 == "Main_Domain" ]] && [[ -f /root/acme/cert.crt && -f /root/acme/private.key && -s /root/acme/cert.crt && -s /root/acme/private.key ]]; then
            caacme=$caacme1
        else
            caacme='无证书申请记录'
        fi
...
yellow "公钥文件crt保存路径：/root/acme/cert.crt"
yellow "密钥文件key保存路径：/root/acme/private.key"
echo
red "========================================================================="
acmeshow
